...
/**
 * Used to notify interested parties that progress has been made for a particular action.
...
public class ProgressEvent extends EventObject {
	 ...
	 */
	protected void setTask(final String newTask) {
		task = newTask;
	}
	...
	/**
	 * The current progress in relation to the maximum, or <code>-1</code> if not known.
	 */
	private float value = -1;

	/**
	 * @return The current progress, in relation to the goal, or <code>-1</code> if not known.
	 * @see #getMaximum
	 */
	 ...
	 * @param newValue The progress in relation to the goal, or <code>-1</code> if not known.
	 * @see #setValue
	 */
	protected void setValue(final float newValue) {
		value = newValue;
	}
	 ...
	 * @see #setValue
	 */
	protected void setMaximum(final float newMaximum) {
		maximum = newMaximum;
	}
	...
	public boolean isFinished() {
		return getValue() >= 0 && getValue() == getMaximum();
	}
	...
	*/
	...
	/**
	 * Creates a progress event with a task and status. If <code>finished</code> is <code>true</code>, the progress event will indicate finished by setting both
	 * the value and the maximum to <code>1</code>.
	 * @param source The source of the event (typically <code>this</code>).
	 * @param task The task being performed.
	 * @param status The status message.
