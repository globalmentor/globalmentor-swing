...
public class SwingText {
	...
	*/
	...
	/**
	 * Determines the ending row model position of the row that contains the specified model position. The component given must have a size to compute the result.
	 ...
	 */
	/**
	 * G**fix if needed public static final int getRowEnd(JTextComponent c, int offs) throws BadLocationException { Rectangle r; if(c instanceof XMLTextPane) {
	 * //TODO newswing r = ((XMLTextPane)c).modelToView(offs, Position.Bias.Backward); //TODO newswing } else { r = c.modelToView(offs); } if (r == null) { return
	 * -1; } int n = c.getDocument().getLength(); int lastOffs = offs; int y = r.y; while ((r != null) && (y == r.y)) { offs = lastOffs; lastOffs += 1; r =
	 * (lastOffs <= n) ? c.modelToView(lastOffs) : null; } return offs; }
	 */
	 ...
	 * @throws BadLocationException Thrown if the element's starting and ending offsets are invalid.
	...
	/**
	 * Inserts an element into a parent element at the given index. If there is a leaf element in the hierarchy of <code>rootElement</code> the ending position of
	 * which is at the beginning insertion position in the model, that leaf element is recreated (as a <code>LeafElement</code>) with the same position it held
	 * before the insert.
	 * <p>
	 * This method is convenient for inserting elements that begin at the ending position of another element, but previous content insertion has, through
	 * automatic position update, caused that element's end to increase. This method allows an element to be inserted after another element, even after a content
	 * insertion.
	 * </p>
	 * @param rootElement The root element in the hierarchy that allows searches for any lower leaf elements that might have ended at the insertion point.
	 * @param parentElement The parent element into which the element should be inserted.
