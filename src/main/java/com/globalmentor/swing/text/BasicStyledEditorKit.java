...
import com.globalmentor.swing.unicode.UnicodePanel;
...
public class BasicStyledEditorKit extends StyledEditorKit implements URIInputStreamable {
	 ...
	 */
	public void setDefaultCursor(final Cursor newDefaultCursor) {
	...
	public Cursor getDefaultLinkCursor() {
		return defaultLinkCursor;
	}
	...
	/**
	 * Determines the cursor to use when over a given link in a particular document. This version return the default link cursor.
	...
	private static final Action[] DEFAULT_ACTIONS = { new DisplayElementTreeAction(DISPLAY_ELEMENT_TREE_ACTION_NAME),
			new DisplayUnicodeTableAction(DISPLAY_UNICODE_TABLE_ACTION_NAME), new PreviousPageAction(PREVIOUS_PAGE_ACTION_NAME),
			new NextPageAction(NEXT_PAGE_ACTION_NAME), new BeginAction(beginAction, false), new EndAction(endAction, false) };
	...
	protected URIInputStreamable getURIInputStreamable() {
	 ...
	 * @param mediaType The media type supported. In some instances, such as <code>text/html</code>, this indicates a default namespace even in the absence of a
	 *          document namespace identfication.
	 * @param uriInputStreamable The source of input streams for resources.
	 */
	 ...
	 */
	public Object clone() {
		return new BasicStyledEditorKit(getMediaType(), getURIInputStreamable());
	} //TODO why do we need this?; make a real clone, or make sure XHTMLEditorKit overrides this

	/**
	 * Returns the MIME type of the data the XML editor kit supports, such as <code>text/plain</code>.
	...
	/**
	 * Create an uninitialized text storage model that is appropriate for this type of editor. This version returns a <code>BasicStyledDocument</code>.
	 * @return The model.
	 */
	 ...
	 * <p>
	 * This version ensures that the caret is not placed on a hidden view.
	 * </p>
	 */
