...
public class XMLAnonymousBlockView extends XMLBlockView //TODO do we even need this class, because wouldn't an anonymous block view with only inline views be an anonymous paragraph view?
{
	...
	/**
	 * Constructs an anomymous block view, specifying whether the view should be allowed to expand to a maximum size for the given axes.
	 * @param element The element this view is responsible for, although in most cases an anonymous view is not responsible for the entire element.
	 * @param attributes The attributes for the anonymous block view.
	 * @param childElements The children of <code>element</code> for which this anonymous view is responsible.
	 * @param axis The tiling axis, either View.X_AXIS or View.Y_AXIS.
	...
	/**
	 * Loads all of the children to initialize the view. This is called by the <a href="#setParent">setParent</a> method. An anonymous block view only represents
	 * a subset of the element's children &mdash; the children we were requested to own. Rather than creating views based upon the element's children, therefore,
	 * this class uses the elements specifed when the class was constructed.
