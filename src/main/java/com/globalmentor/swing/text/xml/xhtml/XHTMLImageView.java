...
/**
 * A view that displays an image, intended to support the XHTML <code>&lt;img&gt;</code> element or the <code>&lt;object&gt;</code> element representing an
 * image. This class is a extension, rewrite, and bug fix of javax.swing.text.html.ImageView by Jens Alfke version 1.40 02/02/00 and is based on code from that
 * class.
...
public class XHTMLImageView extends XMLImageView {
	...
	/**
	 * Creates a new view that represents an XHTML image element. The image is initialized using a different element (which may, for example, be a descendant
	 * element of the original element). This allows us to represent an <code>&lt;object&gt;</code> element but initialize ourselves from one of its children, as
	 * we may recognize the media type of the child element, but not of the element itself.
	 * @param element The element for which to create the view.
	 * @param initializeElement The element to be used for initializing the image information.
	 */
	...
	protected void initialize(Element element) {
		...
		} //ignore any number format exceptions; the width and/or the height will be left at -1 for us to check
	...
	/**
	 * Fetches the attributes to use when rendering. This version returns the attributes used for initialization, which may or may not be the attribute set the
	 * view represents. This allows the view to represent an <code>&lt;object&gt;</code> element but be initialized by one of the object's children of which we
	 * recognize the media type.
	 * @return The attribute set we represent, which may or may not be the attribute set of the element we represent.
	 */
