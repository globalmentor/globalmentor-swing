...
import com.globalmentor.collections.comparators.DefaultOrderComparator;
...
/**
 * Manages a set of actions for an application or a component. The actions are categorized into groups, and action managers may merge or unmerge with actions
 * from other action managers.
 * <p>
 * For consistency and merging, the provided actions should be used if possible to represent top-level menus.
 * </p>
 * <p>
 * To indicate that actions should be visibly separated in any menu or toolbar, an instance of {@link SeparatorAction} should be used.
 * </p>
 * <p>
 * Each menu action may have an action manager order property that will determine its order within its group. Several default orders are provided and should be
 * used for consistency and to facility proper merging. The predefined menu actions use the predefined orders automatically. Actions with no order specified
 * will be sorted in front of those that do, and two menu actions with the same order or no specified order will be sorted in the order in which they were added
 * to the manager.
 * </p>
 * <p>
 * Toolbar actions are not sorted.
 * </p>
...
public class ActionManager implements Cloneable {
	...
	/**
	 * The map of lists of menu actions, keyed to parent actions. Top-level menu actions are keyed to <code>null</code>.
	 */
	private Map<Action, List<Action>> menuActionListMap = new HashMap<Action, List<Action>>(); //TODO would be better using a tree

	/**
	 * Retrieves a list of menu actions for the given parent menu action. If the map has no record of such parent action, it will be added to the map along with a
	 * corresponding list.
	 * @param parentAction The action that serves as the parent of the action, or <code>null</code> if the action is a top-level menu action.
	...
	/**
	 * Adds to the manager an action representing a top-level menu. If the action already exists at the top level, no action occurs.
	...
	/**
	 * Adds to the manager an action representing a menu. If no parent action is specified, the action will be stored as a top-level action. If the action already
	 * exists as a child of the given parent, no action occurs.
	 * @param parentAction The action that serves as the parent of the action, or <code>null</code> if the action is a top-level menu action.
	...
	/**
	 * Returns an iterator to actions representing menus. If no parent action is specified, the actions returned will retpresent top-level actions.
	 * @param parentAction The action that serves as the parent of the actions, or <code>null</code> if an iterator to top-level menu actions should be returned.
	...
	/**
	 * Merges a given action manager with this one, creating a new, merged action manager.
	 * @param actionManager The action manager to merge with this one.
	 * @return The new action manager result of the merger of this action manager and the given action manager.
	 */
	...
	/**
	 * Adds to the manager an action representing a tool. If the action already exists at the top level, no action occurs.
	 ...
	 * @param actionList The list of actions from which the iterator should return actions, or <code>null</code> if no actions are available.
	 * @return A read-only iterator to any actions, or <code>null</code> if there are no actions.
	 * @see ActionMen
	 */
	...
	*/
	...
	/**
	 * Adds toolbar components to the toolbar from the toolbar actions. The components are set not to receive focus.
	 ...
	 * <p>
	 * Most methods should call <code>MenuUtilities.createComponent(Action)</code> or <code>ToolBarUtilities.createComponent(Action)</code>, as those methods
	 * correctly create separators for those contexts.
	 * </p>
	...
	*/
	...
	protected static class ActionMenuOrderComparator extends DefaultOrderComparator<Action> {
		...
		/**
		 * Constructs a comparator to compare actions based upon order or, by default, the order in which they appear in another list.
		 * @param defaultOrderList The list that determines the default order of the actions. This must not a different list than any list being sorted.
		 */
