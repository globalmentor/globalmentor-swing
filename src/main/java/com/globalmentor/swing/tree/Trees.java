...
public class Trees {
	...
	/**
	 * Removes the given tree node from its parent and selects the remaining node, if the remaining node is a <code>DefaultMutableTreeNode</code>.
	 ...
	 * <p>
	 * This method must be called <em>before</em> the node is removed.
	 * </p>
	 * @param treeNode The tree node that is considering deletion.
	 * @return The node that should be selected should this node be removed, or <code>null</code> if there would be no node remaining.
	 */
	...
	/**
	 * Finds the node that is remaining after the child at the given index was. removed. The node returned will be the node at the given index if there is such a
	 * node, or the node at the previous index if one exists, or the parent node if the parent has no more children.
	 * <p>
	...
	/**
	 * Finds a path to the node that is remaining after the child at the given index removed. The path returned will include the node at the given index if there
	 * is such a node, or the node at the previous index if one exists, or the parent node if the parent has no more children.
	 * <p>
	 * If the last component of the parent path is not a <code>TreeNode</code>, the parent path will be returned.
	 * </p>
	 * <p>
	 * This method must be called <em>after</em> the node is removed.
	 * </p>
