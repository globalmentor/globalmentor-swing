...
/**
 * A tree node that can dynamically load its children when needed. Child classes should override {@link #loadChildNodes()}.
 * <p>
 * This class also keeps track of whether there was an error loading nodes, resetting this error status when the nodes are unloaded.
 * </p>
...
public abstract class DynamicTreeNode extends DefaultMutableTreeNode {
	 ...
	 */
	protected void setLoadError(final IOException error) {
		loadError = error;
	}
	...
	/**
	 * @return <code>false</code> so that we won't have to dynamically calculate whether we have children until we need to.
	 */
	...
	/**
	 * @return The number of children, loading the child nodes dynamically if needed.
	...
	/**
	 * Loads children if they haven't already been loaded. If there is an error loading the child nodes, the load error variable will be set.
	 * @see #loadChildNodes
	 */
