...
public class DesktopPaneResourceComponentDecorator<R extends Resource> extends ResourceComponentDecorator<R> implements InternalFrameListener {
	...
	protected VetoableChangeListener getInternalFrameCanCloseListener() {
		return internalFrameCanCloseListener;
	}

	/**
	 * Constructs a decorator for a desktop pane, using the given resource component manager to manager resource components.
		...
		internalFrameCanCloseListener = new VetoableChangeListener() { //create a reusable listener to make sure frames can close
			...
			public void vetoableChange(final PropertyChangeEvent propertyChangeEvent) throws PropertyVetoException {
				...
				if(source instanceof JInternalFrame) { //if the event came from an internal frame
					...
					if(JInternalFrame.IS_CLOSED_PROPERTY.equals(propertyChangeEvent.getPropertyName()) && booleanValue(propertyChangeEvent.getNewValue()) == true) {
	...
	/**
	 * Called when a resource component is added. This implementation adds an internal frame to the desktop pane corresponding to the resource component.
	...
	/**
	 * Called when a resource component is removed. This implementation removes the internal frame associated with the component from the desktop pane.
