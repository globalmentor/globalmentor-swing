...
/**
 * A sequence panel that determines the sequence based upon components that implement the {@link Sequenceable} interface.
 * <p>
 * The class is constructed with an initial component to display. Subsequently, if the component implements {@link Sequenceable}, returns <code>true</code> for
 * {@link Sequenceable#hasNext()}, and the <code>Sequenceable.getNext()</code> method returns a {@link Component}, that component will be displayed next in the
 * sequence, etc. The sequence will be allowed to finish once a component is reached that returns <code>false</code> for {@link Sequenceable#hasNext()}.
 * </p>
 * <p>
...
public class SequenceableSequencePanel extends AbstractComponentSequencePanel //TODO fix or del implements DefaultFocusable
{
	...
	protected LinkedList getSequenceHistoryList() {
		return sequenceHistoryList;
	}

	/** @return A read-only iterator to the visited components in the sequence. */
	public Iterator getSequenceHistoryIterator() {
		return unmodifiableList(sequenceHistoryList).iterator();
	}
	...
	/**
	 * @return <code>true</code> if there is a next component after the current one, which for this version means the current component implements
	 *         <code>Sequenceable</code> and return <code>true</code> for <code>hasNext()</code>.
	...
	/**
	 * @return The next component to be displayed in the sequence, the <code>Component</code> returned from <code>Sequenceable.getNext()</code>.
	...
	/**
	 * @return <code>true</code> if there is a previous component before the current one, in this case if there is at least one component on the stack.
	 */
	...
	/**
	 * @return The previous component to be displayed in the sequence, the top component on the stack.
	 */
