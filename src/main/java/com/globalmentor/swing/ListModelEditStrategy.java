 ...
 * <p>
 * Actual modification of the list will only be performed if the list model implements either {@link DefaultListModel} or {@link List}.
 * </p>
 * <p>
 * If a {@link Modifiable} object is given, its modified status will be set when an item is added, edited, or deleted.
 * </p>
 * <p>
 * For best results, derived classes should override {@link ListModelEditStrategy#getParentComponent()}, along with
 * {@link ListModelEditStrategy#getModifiable()} if needed.
 * </p>
...
public abstract class ListModelEditStrategy<E> {
	...
	/**
	 * The object that represents the list model modification status, or <code>null</code> if modification status should not be indicated.
	 */
	private final Modifiable modifiable;

	/**
	 * @return The object that represents the list model modification status, or <code>null</code> if modification status should not be indicated.
	 */
	protected Modifiable getModifiable() {
		return modifiable;
	}
	 ...
	 * @param parentComponent The component acting as the parent for windows.
	 * @param modifiable The object that represents the list model modification status, or <code>null</code> if modification status should not be indicated.
	 */
	...
	public void delete(final int index) {
		...
		if(index >= 0 && index < listModel.getSize()) { //if a valid index is requested
			...
			if(BasicOptionPane.showConfirmDialog(getParentComponent(), "Are you sure you want to delete the item, \"" + item + "\"?", "Confirm delete",
					BasicOptionPane.OK_CANCEL_OPTION) == BasicOptionPane.OK_OPTION) { //TODO i18n
	...
	/**
	 * Moves an item in the list. The lead selection is moved along with the item if appropriate.
	 ...
	 * @throws InstantiationException Thrown if a class represents an abstract class, an interface, an array class, a primitive type, or void; or if the class has
	 ...
	 * @return The object with the modifications from the edit, or <code>null</code> if the edits should not be accepted.
	 */
	protected abstract E editItem(final E item);

	/**
	 * Deletes an object in the list. This is called so that any physical object backing may be removed, if necessary. This method must not remove the item from
	 * the list.
