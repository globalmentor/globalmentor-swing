...
/**
 * A tree cell renderer that can render different icons and strings for user objects that are RDF resources. The class defaults to using a special icon for RDF
 * literals.
 * <p>
 * If the a tree node is a <code>IconTreeNode</code> that has an icon, its icon is used. If not, specified icons are used for the specified types of RDF
 * resource user objects, if the user object is not an RDF resource, the specified class of user object. If a tree node is not a
 * <code>DefaultMutableTreeCell</code>, the default icons are used from the parent class.
 * </p>
...
public class RDFResourceTreeCellRenderer extends UserObjectTreeCellRenderer {
	 ...
	 * @param icon The icon to register with the resource type, or <code>null</code> if no icon should be associated with the resource type.
	 */
	 ...
	 * @param icon The icon to register with RDF literals, or <code>null</code> if no icon should be associated with RDF literals.
	 */
	 ...
	 * <p>
	 * If the user object is an RDF resource, its type will be returned. If the resource has multiple types it is undefined which type will be returned.
	 * </p>
	 * <p>
	 * If the user object is not an RDF resource, its class will be returned.
	 * </p>
	...
	protected Object getUserObjectKey(final Object userObject) {
		...
		//TODO del			final RDFResource rdfResource=(RDFResource)userObject;  //cast the user object to an RDF resource
			...
			//if this is not an RDF resource or literal
