...
/**
 * Manages icons bundled with an application. Loads icons from the class loader of a given class. This class keeps weak references to the icons it loads so that
 * they may be reused if they have not been garbage collected. This class does have a small overhead of references to icons that are no longer used and have
...
public class IconManager extends ResourceManager {
	...
	/**
	 * Loads an icon resource. The filename may either be the name of a file stored in the same path as the loader class, or the full path to an image file.
	 * <p>
	 * This is a convenience method that calls <code>getResource(String)</code>.
	 * </p>
