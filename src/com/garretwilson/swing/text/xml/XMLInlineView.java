...
//G***del	private boolean Superscript;

	/**The cached subscript value.*/
//G***del	private boolean Subscript;
...
//G***del when works return getElement().getParentElement().getAttributes(); //G***del
	}
	...
	*/
	...
	{
		if(GlyphFont==null)	//G***probably later change to use a list of fonts; TODO use another variable for the synchronization flag, as we probably won't cache the font, anymore
	...
	*/
//G***del	protected void setSuperscript(boolean superscript) {Superscript=superscript;}

	/**Sets whether or not this view represents a subscript.
	@param subscript Whether this view should be a subscript.
	*/
//G***del	protected void setSubscript(boolean subscript) {Subscript=subscript;}
...
//G***fix		throw new StateInvariantError("LabelView needs StyledDocument");
		}
		//G***what if we don't have attributes?
	}
...
*/
	...
	@return The new font.
	*/
	public Font getFont(final String family, final int style, final int size) //G***maybe put this in an XMLGlyphView
	{
...
//G***del			return null;  //show that we can't find a font for the character
	}
...
//G***del	private SimpleAttributeSet AttributeSet;	//G***comment when works
//G***del	Font font=null;	//G***testing

/*G***del
	public Font getFont()
	{
//G***del System.out.println("XMLInlineView.getFont() called.");	//G***del
//G***del		return ((StyledDocument)getDocument()).getFont(getAttributes());	//G***testing
...
//G***del				return getFont(family, style, size);


		Font f = new Font(family, style, size);
System.out.println("XMLInlineView.getFont() called for element "+getElement().getAttributes().getAttribute(XMLCSSStyleConstants.XMLElementNameName)+", resulting in font: "+f);	//G***del
		return f;
	}
*/
		 ...
		 */
/*G***fix
	public void changedUpdate(DocumentEvent e, Shape a, ViewFactory f) {	//G***fix, tidy, comment
...
/*G***fix
		public AttributeSet getAttributes() {
	return AttributeSet;	//G***fix, tidy, comment
		}
*/
	...
	protected void setPropertiesFromAttributes()
	{
		...
		if (attr != null)
		{
			...
			if (d instanceof StyledDocument)
			{
				...
				final XMLCSSSimpleAttributeSet attributeSet=new XMLCSSSimpleAttributeSet();	//G***testing
				StyleConstants.setFontFamily(attributeSet, "Serif");
				StyleConstants.setFontSize(attributeSet, 10);

				font = doc.getFont(attributeSet);	//G***del; testing

//G***fix		font = doc.getFont(attr);
...
/*G***fix
		//convert CSS font properties to Swing attributes *before* calling the parent class, so that it can setup the font correctly
			//G***assuming this is a mutable attribute set sounds dangerous; is there a better way to do this?

		final MutableAttributeSet attributeSet=(MutableAttributeSet)getElement().getAttributes();		//get a reference to our attributes
System.out.println("Setting view properties for element: "+attributeSet.getAttribute(XMLCSSStyleConstants.XMLElementNameName));	//G***del
System.out.println("Current element attributes: "+attributeSet);	//G***del

//G***del		int fontStyle=getFont().getStyle();	//get the style of this font
			//check the font style
		final XMLCSSPrimitiveValue fontStyleProperty=(XMLCSSPrimitiveValue)attributeSet.getAttribute(XMLCSSStyleDeclaration.PROP_FONT_STYLE);	//get the font-style property G***can we be sure this will be a primitive value?
...
System.out.println("Font style is not null: "+fontStyleString);	//G***del
...
//G***del				fontStyle|=Font.ITALIC;	//add italic to this font's list of attributes
...
//G***del				fontStyle&=~Font.ITALIC;	//remove italic from this font's list of attributes
		}
		final XMLCSSPrimitiveValue fontWeightProperty=(XMLCSSPrimitiveValue)attributeSet.getAttribute(XMLCSSStyleDeclaration.PROP_FONT_WEIGHT);	//get the font-weight property G***can we be sure this will be a primitive value?
...
System.out.println("Font weight is not null: "+fontWeightString);	//G***del
...
//G***del				fontStyle|=Font.BOLD;	//add bold to this font's list of attributes
			if(fontWeightString.equals(XMLCSSStyleDeclaration.CSS_FONT_WEIGHT_NORMAL))	//we should not use bold here
;//G***fix				StyleConstants.setBold(attributeSet, false);	//turn bold off
//G***del				fontStyle&=~Font.BOLD;	//remove bold from this font's list of attributes
		}
//G***del		getFont().setStyle(fontStyle);	//change the font's style to reflect any changes we might have made
System.out.println("New element attributes: "+attributeSet);	//G***del
*/

//G***del System.out.println("Setting view properties for element: "+getAttributes().getAttribute(XMLCSSStyleConstants.XMLElementNameName));	//G***del


//G***del when works		super.setPropertiesFromAttributes();	//to the default setting of properties


//G***del System.out.println("After setting inline properties, font is: "+getFont());	//G***del
...
/*G***del
	...
	{
System.out.println("XMLInlineView is testing returning ForcedBreakWeight.");	//G***del
