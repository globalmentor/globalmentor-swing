...
public abstract class XMLImageView extends XMLObjectView implements ImageObserver //TODO fix, MouseListener, MouseMotionListener
{
	 ...
	 */
	protected void setHRef(final String newHRef) {
		href = newHRef;
	}
	...
	*/
	...
	/**
	 * Sets whether or not an view is showing. This version includes the parent functionality, and sets the <code>startedLoading</code> and
	 * <code>finishedLoading</code> variables to <code>false</code> to reflect the fact that the image will need to possibly be reloaded. The internal hard
	 * reference to the image is removed, so that the image memory may be reclaimed if needed.
	...
	protected Image getImage() throws URISyntaxException, IOException {
		...
		if(shownImage != null) //TODO testing; comment
			...
			if(image == null) { //if we have not loaded the image yet, or the image memory has been reclaimed //TODO put all this into a separate function
	...
	/**
	 * Frees the image, if one has been loaded. This allows the garbage collector to reclaim the memory used by the image, which will cause the image to be
	...
	/**
	 * The image currently being painted. This hard reference ensures that, while this view is showing, the image memory will not be reclaimed. When the view is
	 * not showing, this image will be set to false so that the image memory may be recollected if needed.
	 */
	...
	*/
	 ...
	 * @param infoflags The bitwise inclusive OR of the following flags: <code>WIDTH</code>, <code>HEIGHT</code>, <code>PROPERTIES</code>, <code>SOMEBITS</code>,
	 *          <code>FRAMEBITS</code>, <code>ALLBITS</code>, <code>ERROR</code>, <code>ABORT</code>.
	 * @param x The <i>x</i> coordinate.
	 * @param y The <i>y</i> coordinate.
	 * @param width The width.
	 * @param height The height.
	 * @return <code>false</code> if the infoflags indicate that the image is completely loaded; <code>true</code> otherwise.
	 */
