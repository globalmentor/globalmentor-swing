...
import com.globalmentor.swing.text.DefaultViewFactory;
...
/**
 * Table view for elements with the CSS display attribute "table". Modified from {@link javax.swing.text.html.TableView} Copyright 2006 Sun Microsystems, Inc.
...
class XMLTableView extends TableView implements Cloneable {
	...
	public int testGetViewCount() {
		return getViewCount();
	} //TODO del; testing

	public int testGetSpan(int axis, int row) {
		return getSpan(axis, row);
	}
	...
	/**
	 * Sets this view's parent. This is guaranteed to be called before any other methods if the parent view is functioning properly. This is implemented to
	 * forward to the superclass as well as call the <a href="#setPropertiesFromAttributes">setPropertiesFromAttributes</a> method to set the paragraph properties
	 * from the CSS attributes. The call is made at this time to ensure the ability to resolve upward through the parents view attributes.
	 * @param parent The new parent, or null if the view is being removed from a parent it was previously added to.
	 */
	...
	/**
	 * Calculates the requirements of the table along the minor axis (i.e. the major axis of the rows). This is implemented to provide the superclass behavior and
	...
	*/
	...
	/**
	 * Returns the <code>ViewFactory</code> to use for the table. This is implemented to create a new factory that creates special views for special internal
	 * table elements and uses the given view factory to create all other views.
	...
	*/
	...
	/**
	 * Determines the maximum span for the view along an axis. This implementation returns the preferred view, as the table will not stretch beyond that amount.
	...
	*/
	...
	/**
	 * Renders using the given rendering surface and area on that surface. This is implemented to delegate to the css box painter to paint the border and
	...
	*/
	...
	/**
	 * Returns the the view factory, which is our <code>XMLTableFactory</code> that creates table-related views or delegates the responsibility to the default
	 * view factory.
	...
	/**
	 * Creates a shallow copy. This is used by the <code>createFragment()</code> and <code>breakView()</code> methods.
	 ...
	 * given arrays which represent the allocations to the children along the minor axis. This is called by the superclass whenever the layout needs to be updated
	 * along the minor axis.
	 * <p>
	 * This is implemented to call the <a href="#layoutColumns">layoutColumns</a> method, and then forward to the superclass to actually carry out the layout of
	 * the tables rows.
	 *
	...
	/**
	 * Calculate the requirements for the minor axis. This is called by the superclass whenever the requirements need to be updated (i.e. a preferenceChanged was
	 * messaged through this view).
	 * <p>
	...
	*/
	...
	/**
	 * Calculate the requirements for each column. The calculation is done as two passes over the table. The table cells that occupy a single column are scanned
	 * first to determine the maximum of minimum, preferred, and maximum spans along the give axis. Table cells that span multiple columns are excluded from the
	 * first pass. A second pass is made to determine if the cells that span multiple columns are satisfied. If the column requirements are not satisified, the
	 * needs of the multi-column cell is mixed into the existing column requirements. The calculation of the multi-column distribution is based upon the
	 * proportions of the existing column requirements and taking into consideration any constraining maximums.
	 */
	...
	/**
	 * Fetches the child view that represents the given position in the model. This is implemented to walk through the children looking for a range that contains
	 * the given position. In this view the children do not necessarily have a one to one mapping with the child elements.
	 *
	...
	/**
	 * An html row. This adds storage of the appropriate css attributes to the superclass behavior.
	 */
	/**
	 * A row in an XML table. This version correctly gets attributes from CSS attribute properties
	...
	class XMLTableRowView extends TableView.TableRow {
		...
		/**
		 * Sets the parent of the view. This implementation also indicates that the preferences have changed and asks each child to reset the parent.
		...
		*/
		...
		public int getBreakWeight(int axis, float pos, float len) {
			...
			if(axis == Y_AXIS) { //if they want to break along the Y axis
				...
				//if they want to break along another axis besides the one we know about
				return super.getBreakWeight(axis, pos, len); //return the default break weight
		}
		...
		public View breakView(int axis, int p0, float pos, float len) {
			if(axis == Y_AXIS) { //if they want to break along the Y axis
				...
				if(len >= getPreferredSpan(axis)) //if the break is as large or larger than the view
					...
					int startOffset = p0; //we'll continually update this as we create new child view fragments
					int childIndex; //start looking at the first child to find one that can be included in our break
					for(childIndex = 0; childIndex < childViewCount && getView(childIndex).getEndOffset() <= startOffset; ++childIndex)
						; //find the first child that ends after our first model location
		...
		/**
		 * Loads all of the children to initialize the view. This is called by the <a href="#setParent">setParent</a> method. A block view knows that, should there
		 * be both inline and block children, the views for the inline children should not be created normally but should be wrapped in one or more anonymous views.
		 * Furthermore, inline views consisting only of whitespace will be given hidden views.
		 * @param viewFactory The view factory.
		 * @see CompositeView#setParent
		 */
		...
		/**
		 * Perform layout for the major axis of the box (i.e. the axis that it represents). The results of the layout should be placed in the given arrays which
		 * represent the allocations to the children along the major axis.
		 * <p>
		...
		/**
		 * Perform layout for the minor axis of the box (i.e. the axis orthoginal to the axis that it represents). The results of the layout should be placed in the
		 * given arrays which represent the allocations to the children along the minor axis. This is called by the superclass whenever the layout needs to be
		...
		/**
		 * Fetches the child view that represents the given position in the model. This is implemented to walk through the children looking for a range that
		 * contains the given position. In this view the children do not necessarily have a one to one mapping with the child elements.
		 *
		 * @param pos the search position >= 0
		 * @param a the allocation to the table on entry, and the allocation of the view containing the position on exit
		 * @returns the view representing the given position, or null if there isn't one
	...
	/**
	 * This is a custom view factory meant to create views for elements inside a table. Its constructor take a reference to the <code>ViewFactory</code> already
	 * in use, and this is used to create views for any unknown elements, thereby keeping the default action for non-table elements. The only need for this class
	 * seems to be that outside classes can't directly create such classes as <code>XMLTableRow</code>, and since <code>XMLTableRow</code> must be internal to
	 * inherit from <code>TableView.TableRow</code>, each row must be created within <code>XMLTableView</code>.
	 */
	class XMLTableFactory extends DefaultViewFactory {
		...
		/**
		 * Creates a view for the table rows. If the element does not represent a table row, the request will be delegated to the fallback view factory.
