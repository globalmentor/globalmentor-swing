...
/**
 * A view that can represent not only a fragment of a block view but of other views as well. It is correctly marked as a fragment view by implementing
 * {@link FragmentView}, and keeps track of whether the view is the first fragment. //TODO in this and all fragment views, fix the replace() method so that it
 * reparents the view to the whole view if it was originally a child of the whole view; then, update the CompositeBoxView break strategy and see if we can
 * remove the reparenting; make sure that any reparenting works with flow views with their logical view pools
...
public class XMLFragmentBlockView extends XMLBlockView implements FragmentView {
	...
	/**
	 * Returns the attributes to use for this view. This version returns the attributes of the whole view.
	...
	/**
	 * Fragment views do not load their own children. Instead, the method fragmenting the original view will manually add children to this view.
