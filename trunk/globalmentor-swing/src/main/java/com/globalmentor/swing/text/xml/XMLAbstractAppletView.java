...
/**
 * A view that displays an applet. Implements {@link AppletStub} so that it can manage requests to the environment made by the applet. A class must be derived
 * from this abstract class that correctly sets the class name, as well as correctly sets the correct width and height. Although not required, it is recommended
 * that the derived clas override loadParameters() to correctly set the applet parameters, although it is conceivable that a particular implementation will not
 * need parameters for the associated applet to function correctly.
...
public abstract class XMLAbstractAppletView extends XMLAbstractComponentView implements AppletStub {
	...
	/**
	 * Convenience function to return the component on which this view is based, automatically casting the component to an applet.
	 * @return The applet which this view represents.
	 * @see XMLComponentView#getComponent
	 */
	public Applet getApplet() {
		return (Applet)getComponent();
	}

	/**
	 * The href of the class to load, including any ending ".class" extension, relative to the document base URI.
	 */
	private String classHRef = null;

	/**
	 * @return The href of the class to load, including any ending ".class" extension, relative to the document base URI.
	 */
	...
	/**
	 * Sets the path of the Java applet class to load. This path will be used both to determine the applet codebase and to load the applet. This method does
	 * nothing if the Java class has already been created.
	 * @param newClassHRef The href of the class to load, including any ending ".class" extension, relative to the document base URI.
	 * @see #setClassName
	 */
	...
	/**
	 * Sets the name of the Java applet class to load. This method does nothing if the Java class has already been created.
	...
	protected boolean isParametersLoaded() {
	 ...
	 * @return The parameter with the specified name, or <code>null</code> if that parameter does not exist.
	 */
	...
	/**
	 * Sets a parameter by its name. If a parameter with that name already exists, it will be replaced.
	...
	/**
	 * Indicates whether the applet is active -- from applet <code>start()</code> to <code>stop()</code>.
	 */
	...
	/**
	 * Called by the garbage collector when this class is no longer used. This version stops the applet, if present, and calls the applet's <code>destroy()</code>
	 * method. This method, as with any <code>finalize()</code> method in Java, is not guaranteed to ever be called.
	 * @throws Throwable Any <code>Exception</code> thrown by the method. This is ignored by the garbage collector.
	 * @see #stop
	 * @see Applet#stop
	 * @see Applet#destroy
	 */
	...
	/**
	 * Called when the view is being hidden by a parent that hides views, such as a paged view. This implementation includes the parent funtionality, as well as
	 * starting or stopping the associated applet as appropriate.
	...
	public void setShowing(final boolean showing) {
		...
		if(showing) //if we should be showing the applet
			...
			//if we shouldn't be showing the applet
			stop(); //stop the applet if needed TODO probably change to setActive()
	}

	/**
	 * Sets the parent of the view. This is reimplemented to provide the superclass behavior as well as loading whatever parameter child elements (if any) the
	 * applet has. Once parameters have been set, future calls to this method will not cause the parameters to be reloaded.
	...
	/**
	 * Unconditionally loads all parameters from child elements. Called by the <code>setParent()</code> method.
	 * @see #setParent
	 */
	protected abstract void loadParameters();

	/**
	 * Creates an applet component for displaying according to the classname that has been set.
	 * @return An applet meant for displaying, or if an error occurs loading the applet, a component indicatin the error. ///TODO fix @throws ClassCastException
	 *         Thrown if the class returned by
	 * @see #getClass
	 * @see #getClassName
	 */
	...
	/**
	 * Determines if the applet is active. An applet is active just before its <code>start</code> method is called. It becomes inactive just before its
	...
	/**
	 * Returns the value of the named parameter in a <code>&lt;param&gt;</code> element.
