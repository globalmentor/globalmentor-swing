...
public class TabbedPaneResourceComponentDecorator<R extends Resource> extends ResourceComponentDecorator<R> {
	...
	/**
	 * Constructs a decorator for a tabbed pane, using the given resource component manager to manager resource components.
		...
		tabbedPane.addChangeListener(new ChangeListener() { //listen for tab changes, and update the selected resource in response
					...
					public void stateChanged(final ChangeEvent changeEvent) { //when the tab changes
						...
						if(selectedComponent != null) { //if there is a selected tab
							...
							final ResourceComponentManager<R>.ResourceComponentState resourceComponentState = resourceComponentManager
									.getResourceComponentState(selectedComponent);
	...
	/**
	 * Called when a resource component is added. This implementation adds a tab to the tabbed pane corresponding to the resource component.
	...
	/**
	 * Called when a resource component is removed. This implementation removes the resource component from the tabbed pane.
