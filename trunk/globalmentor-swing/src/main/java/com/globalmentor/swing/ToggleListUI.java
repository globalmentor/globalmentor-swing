...
public class ToggleListUI extends BasicListUI {
	...
	private static class IncrementLeadSelectionAction extends AbstractAction {
		...
		/**
		 * Returns the next index to select. This is based on the lead selected index and the <code>amount</code> ivar.
		 */
		...
		/**
		 * Invokes <code>getNextIndex</code> to determine the next index to select. If the index is valid (not -1 and < size of the model), this will either: move
	...
	/**
	 * Creates a delegate that implements <code>MouseInputListener</code>. This version returns a mouse input listener that allows toggling of list items.
	 */
