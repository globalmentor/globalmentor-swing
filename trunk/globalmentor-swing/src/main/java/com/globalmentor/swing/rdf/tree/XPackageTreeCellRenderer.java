...
/**
 * A tree cell renderer that can render different icons and strings for user objects that are XPackage RDF resources.
 * <p>
 * If the a tree node is an {@link IconTreeNode} that has an icon, its icon is used. If not, specified icons are used for the specified types of RDF resource
 * user objects, using first the XPackage content type and then, if there is no content type, the RDF resource type. If the user object is not an RDF resource,
 * the specified class of user object is used to find an icon. If a tree node is not a {@link DefaultMutableTreeNode}, the default icons are used from the
 * parent class.
 * </p>
...
public class XPackageTreeCellRenderer extends RDFResourceTreeCellRenderer {
	 ...
	 * @param icon The icon to register with the media type, or <code>null</code> if no icon should be associated with the media type.
	 */
	 ...
	 * <p>
	 * If the user object is an RDF resource, the base content type of the content type will be returned. If the resource has no content type, the RDF resource
	 * type will be returned. If the resource has multiple types it is undefined which type will be returned.
	 * </p>
	 * <p>
