 ...
 * <p>
 * This toolbar maintains whether contained buttons should display text. When a button is added, whether the button has text will depend on the status of the
 * toolbar <code>buttonTextVisible</code> property unless the button has no icon. When this property is changed using {@link #setButtonTextVisible(boolean)},
 * all buttons on the toolbar change to match the new setting. The default value of the <code>buttonTextVisible</code> property is <code>true</code>.
 * </p>
 * <p>
 * If an added button's action defines an accelerator, it is added to the parent, either when the button is added to the toolbar or when the toolbar is added to
...
public class BasicToolBar extends JToolBar {
	 ...
	 * @param newVisible <code>true</code> if buttons should display text, else <code>false</code>.
	 */
	 ...
	 * <p>
	 * If a button is added, whether it has text will depend on the state of the <code>buttonTextVisible</code> property.
	 * </p>
	 ...
	 * </p>
	 * <p>
	 * If a button's action has an accelerator key, it will be installed in the toolbar's parent container, if that parent is a <code>JComponent</code>.
	 * </p>
	...
	/**
	 * Adds accelerators for all contained buttons with actions that have accelerator keys. If the parent component is not a <code>JComponent</code>, or if there
	 * is no parent component, no action occurs.
	 * @see #addAccelerator(Action)
	 */
	 ...
	 * <p>
	 * This version adds any accelerators contained in any toolbar button actions to the parent component, as this method is called whenever the toolbar has been
	 * added to a parent.
	 * </p>
	 * @see #addAccelerators()
	 */
	 ...
	 * @return The child component, such as a button, that represents the given action, or <code>null</code> if no child component represents the given action.
	 */
