 ...
 * <p>
 * Although this class overrides {@link FlowView}, much of the layout is actually a replication of code in {@link BoxView}, which is necessary because the
 * latter hides a lot of layout-related variables, which we need to update since we extend some of the functionality of {@link BoxView}.
 * </p>
 * <p>
 * This class, before hiding a page of views, informs each view that implements <code>ViewHidable</code> that it is about to be hidden.
 * </p>
 * @see ViewHidable
 * @see FlowView
 */
public class XMLPagedView extends FlowView {
	...
	protected PaginateStrategy getStrategy() {
		return (PaginateStrategy)strategy;
	}
	...
	*/
	...
	protected PagePoolView getPagePoolView() {
		return (PagePoolView)layoutPool;
	}
	 ...
	 */
	protected void setPagePoolView(final View logicalView) {
		layoutPool = logicalView;
	}
	...
	public int getPageIndex() {
		return getPageCount() > 0 ? pageIndex : -1;
	}

	/**
	 * Sets the new current logical page index. If necessary, the page index is modified so that the displayed page is within the page range and that the page
	 * index represents the first of any set of displayed pages.
	...
	/**
	 * Determines the base page index of the set of pages including the given index. In this implementation, the base page index for the first page is always
	 * zero. If there are no pages, -1 will be returned. If the page index is less than zero, the zero page index will be returned. If the page index is greater
	 * than the number of available pages, the base index of the last logical page will be returned.
	...
	/**
	 * Renders using the given rendering surface and area on that surface. This function only paints the currently selected page.
		...
		if(!isPaginating()) {
			...
			for(int pageIndex = pageBeginIndex; pageIndex < pageEndIndex && isLaidOut(pageIndex); ++pageIndex) { //look at each page (although this may be more pages than we have to paint)
				...
				if(pageIndex < pageEndIndex - 1) { //if this isn't the last page, draw the vertical divider between pages
					...
					paintSpineSection(graphics2D, tempRectangle.x + tempRectangle.width, tempRectangle.y, tempRectangle.height, outerSpineHalfWidth, Color.lightGray,
							Color.white); //paint this section of the spine
					paintSpineSection(graphics2D, tempRectangle.x + tempRectangle.width, tempRectangle.y, tempRectangle.height, -outerSpineHalfWidth, Color.lightGray,
							Color.white); //paint this section of the spine
					paintSpineSection(graphics2D, tempRectangle.x + tempRectangle.width, tempRectangle.y, tempRectangle.height, innerSpineHalfWidth, Color.darkGray,
							Color.lightGray); //paint this section of the spine
					paintSpineSection(graphics2D, tempRectangle.x + tempRectangle.width, tempRectangle.y, tempRectangle.height, -innerSpineHalfWidth, Color.darkGray,
							Color.lightGray); //paint this section of the spine
				}
				if(isLaidOut(pageIndex)) { //if we actually have a page view for this page, and it has been laid out
					...
					if(pageIndex == pageEndIndex - 1) //if we're on the last page
						...
						//if we're not on the last page TODO put code here for the middle of a three-page spread
	...
	protected void paintSpineSection(final Graphics2D graphics2D, final int topX, final int topY, final int height, int horizontalDelta, final Color color1,
			final Color color2) {
	...
	/**
	 * @return <code>true</code> if this view supports threading in some form and there the layout process is currently occurring.
	 */
	...
	*/
	...
	/**
	 * Loads all of the children to initialize the view. This version ensures that our container knows about this paged view.
	...
	/**
	 * Sets the size of the view, causing layout to occur if needed. This version updates the page size and then calls the layout method with the page size inside
	 * of the insets.
	...
	*/
	...
	/**
	 * The class that performs layout. This implementation runs this class in a separate thread.
	...
	/**
	 * Performs layout for the minor axis of the view (the page width). This version assumes all pages have the same width and are layed out
	...
	/**
	 * Performs layout for the minor axis of the view (the page height). This version assumes all pages are the same offset and height.
	 * @param targetSpan The total span given to the view.
	 * @param axis The axis being layed out.
	 * @param offsets The offsets from the origin of the view for each of the child views to be calculated.
	 * @param spans The span of each child view to be calculated.
	...
	/**
	 * * Provides a mapping from the document model coordinate space to the coordinate space of the view mapped to it. This makes sure the allocation is valid
	 * before letting the superclass do its thing.
	 *
	...
	*/
	...
	public int viewToModel(float x, float y, Shape a, Position.Bias[] bias) {
		...
		//TODO del Log.trace("XMLPagedView.viewToModel() is before");
			...
			} //ignore bad location errors
			catch(IllegalArgumentException iae) {
			} //ignore illegal argument errors
	...
	/**
	 * Determines the desired alignment for this view along an axis. This is implemented to give the alignment to the center of the first row along the y axis,
	...
	/**
	 * Internally-created view that holds the view representing child views arranged in pages. This class descends from <code>ContainerView</code>, which
	 * correctly returns starting and ending offsets based upon the child views it contains, not the element it represents.
	 * @author Garret Wilson
	 */
	protected class Page extends ContainerBoxView {
		...
		/**
		 * Sets the cached properties from the attributes. This version forces the margins to a particular size.
		 */
		/*TODO fix
		...
		*/
		/**
		 * Each page does not need to fill its children, since its parent <code>XMLPagedView</code> will load its children with the views it created. This function
	...
	public class DebugFlowStrategy extends FlowStrategy {
		...
		/**
		 * Gives notification that something was inserted into the document in a location that the given flow view is responsible for. The strategy should update
		 * the appropriate changed region (which depends upon the strategy used for repair).
		 *
		 * @param e the change information from the associated document
		 * @param alloc the current allocation of the view inside of the insets. This value will be null if the view has not yet been displayed.
		...
		/**
		 * Update the flow on the given FlowView. By default, this causes all of the rows (child views) to be rebuilt to match the given constraints for each row.
		 * This is called by a FlowView.layout to update the child views in the flow.
		 *
		...
		/**
		 * Creates a row of views that will fit within the layout span of the row. This is called by the layout method. This is implemented to fill the row by
		 * repeatedly calling the createView method until the available span has been exhausted, a forced break was encountered, or the createView method returned
		 * null. If the remaining span was exhaused, the adjustRow method will be called to perform adjustments to the row to try and make it fit into the given
		 * span.
		 *
		 * @param rowIndex the index of the row to fill in with views. The row is assumed to be empty on entry.
		...
		/**
		 * Adjusts the given row if possible to fit within the layout span. By default this will try to find the highest break weight possible nearest the end of
		...
		/**
		 * Creates a view that can be used to represent the current piece of the flow. This can be either an entire view from the logical view, or a fragment of the
		 * logical view.
		 *
	...
	public class PaginateStrategy extends DebugFlowStrategy {
		...
		public void layout(final FlowView flowView) {
			...
			try {
				...
				getPagePoolView().setSize((int)getPageWidth() - getPageLeftInset() - getPageRightInset(),
						(int)getPageHeight() - getPageTopInset() - getPageBottomInset()); //set the size of the page pool to be exactly the size of the displayed page; giving insets to the page pool results in incorrect layout
		...
		/**
		 * Creates a row of views that will fit within the layout span of the row. This is called by the layout method.
		 * @param flowView The view to reflow.
		 * @param rowIndex The index of the row to fill in with views. The row is assumed to be empty on entry.
		 * @param pos The current position in the children of this views element from which to start.
		 * @return The position to start the next row.
		...
		protected int layoutRow(final FlowView flowView, final int rowIndex, final int pos) {
			...
			fireMadeProgress(new ProgressEvent(flowView, PAGINATE_TASK, "Paginating page " + (rowIndex + 1) + " of ~" + Math.round(estimatedRowCount) + "...",
					rowIndex, estimatedRowCount)); //show that we are paginating the specified page, and the number of pages we guess there will be TODO i18n
	...
	/**
	 * The logical view of child elements which will be paginated into pages. Because pagination is a meta-flowing that requires underlying layout on paragraphs
	 * and other flowing views, this view allows for pre-pagination by setting its size.
	 * @author Garret Wilson
	 */
	protected class PagePoolView extends ContainerBoxView {
		...
		/**
		 * Loads the children into this view pool. Only the children within the view's range (specified in the constructor) will be loaded.
		 ...
		 * parent page if the child could fit without breaking.
