...
/**
 * Class that fires action events when a node is double-clicked or the Enter key is pressed. This class will be superceded if a future version of {@link JTree}
...
public class ActionTree extends JTree {
	...
	/**
	 * Constructs a tree that displays the elements in the specified <code>Vector</code>.
		...
		for(int i = listeners.length - 2; i >= 0; i -= 2) { //look at all the listener types
			if(listeners[i] == ActionListener.class) { //if this is an action listener
				if(fireActionEvent == null) { //if we haven't yet created an event to fire
					...
					fireActionEvent = new ActionEvent(this, ActionEvent.ACTION_PERFORMED, actionCommand, actionEvent.getModifiers()); //create a new event for firing, using the parameters of the passed event
				}
