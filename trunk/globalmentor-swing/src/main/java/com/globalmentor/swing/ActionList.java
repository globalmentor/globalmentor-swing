...
/**
 * Class that fires action events when an item is double-clicked or the Enter key is pressed. This class will be superceded if a future version of {@link JList}
 * handles these key/mouse bindings for the correct UI.
 * <p>
 * This class was created from consulting the JDK 1.3.1 API documentation for {@link JList} and {@link JButton}, along with the sample <code>ActionJList</code>
 * code at <a href="http://www.rgagnon.com/javadetails/java-0201.html">http://www.rgagnon.com/javadetails/java-0201.html</a>.
 * </p>
...
public class ActionList extends JList {
	 ...
	 * @throws IllegalArgumentException Thrown if <code>dataModel</code> is <code>null</code>.
	 */
	...
	/**
	 * Constructs a list that displays the elements in the specified <code>Vector</code>.
		...
		for(int i = listeners.length - 2; i >= 0; i -= 2) { //look at all the listener types
			if(listeners[i] == ActionListener.class) { //if this is an action listener
				if(fireActionEvent == null) { //if we haven't yet created an event to fire
					...
					fireActionEvent = new ActionEvent(ActionList.this, ActionEvent.ACTION_PERFORMED, actionCommand, actionEvent.getModifiers()); //create a new event for firing, using the parameters of the passed event
				}
