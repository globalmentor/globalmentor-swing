...
public abstract class URITargetAction extends TargetAction {
	...
	public URI getURITarget() {
		return (URI)getTarget();
	}

	/**
	 * Sets the URI target of the action.
	 * @param newURITarget The new action target, or <code>null</code> if there should be no target.
	 */
	protected void setURITarget(final URI newURITarget) {
		setTarget(newURITarget);
	}
	 ...
	 * @param name The name description of the action.
	 * @param target The new action target, or <code>null</code> if there should be no target.
	 */
