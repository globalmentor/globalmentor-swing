 ...
 * <p>
 * An implementing class may consider the data in each view to be loaded and stored separately, in which case {@link ModelViewablePanel#loadModel(int)} and
 * {@link ModelViewablePanel#saveModel(int)} should be used for the appropriate views. The implementation may instead decide that all data is saved and loaded
 * together, in which case {@link ModelViewablePanel#loadModel()} and {@link ModelViewablePanel#saveModel()} should be used to load or save all data in all
 * views at the same time. An implementation may use a hybrid of these paradigms, for example storing views of all the data in some views using the
 * view-specific loading and saving methods, and using the generic loading and save methods for views that contain portions of the data.
 * </p>
 * <p>
 * Bound properties:
 * </p>
...
public abstract class ModelViewablePanel<M> extends ModelPanel<M> implements ModelViewable {
	 ...
	 */
	public void setDefaultDataView(final int dataView) {
	...
	/**
	 * Stores the current data being edited to the model, if necessary. If no model is being edited or there is no valid view, no action occurs.
	 * @param modelView The view of the data, such as <code>SUMMARY_MODEL_VIEW</code>.
	 * @throws IOException Thrown if there was an error saving the model.
	 */
