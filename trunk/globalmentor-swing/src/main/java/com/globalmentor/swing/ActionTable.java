...
/**
 * Class that fires action events when a cell is double-clicked or the Enter key is pressed. This class will be superceded if a future version of {@link JTable}
 * handles these key/mouse bindings for the correct UI.
 * <p>
 * It is unknown whether this implementation correctly handles reordered columns.
 * </p>
...
public class ActionTable extends JTable {
	...
	/**
	 * Constructs a table that is initialized with <code>dm</code> as the data model, <code>cm</code> as the column model, and <code>sm</code> as the selection
	 * model. If any of the parameters are <code>null</code> this method will initialize the table with the corresponding default model.
	 * @param dm The data model for the table.
	 * @param cm The column model for the table.
	...
	/**
	 * Constructs a table to display the values in the <code>Vector</code> of <code>Vectors</code>, <code>rowData</code>, with column names,
		...
		for(int i = listeners.length - 2; i >= 0; i -= 2) { //look at all the listener types
			if(listeners[i] == ActionListener.class) { //if this is an action listener
				if(fireActionEvent == null) { //if we haven't yet created an event to fire
					...
					fireActionEvent = new ActionEvent(ActionTable.this, ActionEvent.ACTION_PERFORMED, actionCommand, actionEvent.getModifiers()); //create a new event for firing, using the parameters of the passed event
				}
