...
import com.globalmentor.awt.EventQueues;
 ...
 * <p>
 * This component defines the left and right arrow keys and the PageUp/PageDown keys for page navigation.
 * </p>
 * <p>
 * This class also serves as a context that can keep track of applets and load their audio clips and images.
 * </p>
 * @author Garret Wilson
 * @see javax.swing.JTextPane
 * @see com.globalmentor.swing.event.PageEvent
 * @see com.globalmentor.swing.event.PageListener
 */
public class XMLTextPane extends JTextPane implements AppletContext, /*TODO del when works KeyListener, *//*TODO del JDK1.5 MouseMotionListener,*/
		PageListener, ProgressListener, URIAccessible //TODO eventually change this to a BasicTextPane
{
	...
	public void setAsynchronousLoad(final boolean newAsynchronous) {
		asynchronousLoad = newAsynchronous;
	}
	...
	protected URIInputStreamable getBaseURIInputStreamable() {
		return baseURIInputStreamable;
	}
	...
	protected static final KeyBinding[] BASIC_KEY_BINDINGS = {
			...
			new KeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_E, InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK),
	...
	/**
	 * Whether text in this component is antialiased. We keep a local copy stored so that we can update any new text pane when it is installed.
	 */
	...
	/**
	 * The position after which searching will begin, or -1 if searching has not been performed or no match was found.
	 */
	...
	/**
	 * Updates the search offset and length, marking where searching last found a match. Removes all search highlighters and adds the appropriate highlighter
	 * based on the new search position.
	 * @param newSearchOffset The position after which searching should begin.
	 * @param newSearchLength The length of the match.
	 * @throws BadLocationException Thrown if the given search offset and/or length do not represent a valid location in the document.
	...
	/**
	 * Resets the search position so that the next search will be performed as if no previous searches have been performed.
	 */
	...
	/**
	 * @return The length of the last search match, or zero if there has been no match.
	 */
	...
	/**
	 * Registers a view factory for a particular namespace URI. These will be used by any installed <code>XMLEditorKit</code>.
	...
	/**
	 * Registers a link controller for a particular namespace URI. These will be used by any installed <code>XMLEditorKit</code>.
	 * @param namespaceURI The namespace URI that identifies the namespace, elements in which will use the given link controllers to generate link entry, exit,
	 ...
	 * @return A link controller for creating views for elements in the given namepace, or <code>null</code> if no view factory has been registered for the given
	 *         namespace.
	 */
	...
	/**
	 * Constructs a new <code>XMLTextPane</code> with a specified XML document model, and the editor kit is set to a new instance of <code>XMLEditorKit</code>.
	 * @param doc The document model, a descendant of com.globalmentor.swing.text.xml.XMLDocument.
	 * @see com.globalmentor.swing.text.xml.XMLEditorKit
	 * @see com.globalmentor.swing.text.xml.XMLDocument
	 */
	...
	/**
	 * Constructs a new <code>XMLTextPane</code> and the editor kit is set to a new instance of <code>XMLEditorKit</code>. The document model is set to
	 * <code>null</code>.
	 * @see com.globalmentor.swing.text.xml.XMLEditorKit
	 */
	public XMLTextPane() {
	...
	/**
	 * Searches the view hierarchy for an XMLPagedView, and uses the first one it finds as this pane's paged view.
	 * @see XMLPagedView
	...
	*/
	...
	/**
	 * Searches the view hierarchy for an XMLPagedView, and uses the first one it finds as this pane's paged view. This function is called from fetchPagedView().
	 * @see XMLPagedView
	 * @see XMLTextPane#setPagedView
	 * @see XMLTextPane#fetchPagedView
	 */
	...
	*/
	...
	/**
	 * Associates the editor with a text document. This document must be a com.globalmentor.swing.text.xml.XMLDocument.
	 * @param doc The document to display and/or edit.
	 * @throws IllegalArgumentException Thrown if doc is not a descendant of com.globalmentor.swing.text.xml.XMLDocument.
	 * @see XMLTextPane#setXMLDocument
	 */
	/**
	 * TODO we can't have this because we the default JTextPane method creates a default editor kit that creates a default document before we can do anything
	 * about it public void setDocument(Document doc) { if(doc instanceof XMLDocument) //if this is an XMLDocument, which we require super.setDocument(doc); //set
	 * the document normally else //if this isn't an XMLDocument throw new IllegalArgumentException("Model must be XMLDocument"); //throw an exception stating the
	 * error }
	 */
	 ...
	 * @throws IllegalArgumentException Thrown if doc can't be narrowed to a <code>StyledDocument</code> which is the required type of model for this text
	 *           component.
	 */
	...
	/**
	 * Returns the model associated with the editor. This is a convenience method which does the required casting to com.globalmentor.swing.text.xml.XMLDocument.
	...
	/**
	 * Creates and returns the editor kit to use by default. This version returns a com.globalmentor.swing.text.BasicStyledEditorKit
	 ...
	 * <p>
	 * The base URI is also updated in the currently loaded document.
	 * </p>
	 ...
	 * @return The location against which to resolve relative URIs, or <code>null</code> if there is no base URI.
	 */
	...
	/**
	 * Returns an input stream for the given URI. The calling class has the responsibility for closing the input stream. This implementation delegates to the
	 * internal <code>URIInputStreamable</code>.
	...
	/**
	 * Returns an output stream for the given URI. The calling class has the responsibility for closing the output stream. This implementation delegates to the
	 * internal <code>URIOutputStreamable</code>.
	 * @param uri A URI to a resource.
	 ...
	 * <p>
	 * If the XML has no root element, it will be wrapped in <code>&lt;div&gt;...&lt;/div&gt;</code>. This means that simple text will also be considered and
	 * processed as XML. (It is usually desired that the content type be set to "application/xhtml+xml").
	 * </p>
	...
	public void setText(String text) {
		...
		} //if we don't recognize the content type, just set the text normally
		super.setText(text); //set the text normally
	}

	/**
	 * Returns the text contained in this <code>TextComponent</code> in terms of the content type of this editor. If an exception is thrown while attempting to
	 * retrieve the text, <code>null</code> will be returned. This is implemented to call <code>JTextComponent.write</code> with a <code>StringWriter</code>.
	 *
	 * @return the text
	 * @see #setText
	 */
	...
	*/
	...
	/**
	 * Initializes from a URL to a file, which could be a document or, for OEB, a package or a .zip file. This creates a model of the type appropriate for the
	 * component (such as an OEB document) and initializes the model using the appropriate editor kit.
	...
	/**
	 * Initializes from a URI to a resource, which could be a document or, for OEB, a package or a .zip file. This creates a model of the type appropriate for the
	 * component (such as an OEB document) and initializes the model using the appropriate editor kit.
	 * <p>
	 * This method must be called from inside the AWT event thread.
	 * </p>
	 * <p>
	 * The current input stream locator is used.
	 * </p>
	 ...
	 * component (such as an OEB document) and initializes the model using the appropriate editor kit.
	 * <p>
	 * This method must be called from inside the AWT thread.
	 * </p>
	 * @param uri The URI of the resource which has the information to load.
	...
	/**
	 * Initializes from a URI with its input stream already provided. This creates a model of the type appropriate for the component (such as an OEB document) and
	 * initializes the model using the current editor kit.
	 * <p>
	 * The <code>URIInputStreamable</code> and editor kit should have already been initialized.
	 * </p>
	 * <p>
	 * This method must be called from inside the AWT thread.
	 * </p>
	...
	protected class DocumentLoader extends Thread {
		...
		/**
		 * Constructs a thread for asynchronously loading a document from an input stream.
	 ...
	 * <p>
	 * This version calls the URI version.
	 * </p>
	 * @param url The URI of the page.
	 * @deprecated Replaced with setPage(URI) because in ambiguities in URL reserved character encoding.
	 * @see #getStream(URI)
	 */
	/*TODO del
	...
	*/
	...
	/**
	 * Fetches a stream for the given URL, which is about to be loaded by the <code>setPage</code> method. This method has the the side effect of establishing the
	 * content type, and therefore setting the appropriate <code>EditorKit</code> to use for loading the stream. This implementation is only provided for
	 * completeness; the <code>setPage()</code> method uses its own determination of the stream and sets the content type.
	...
	/**
	 * This method initializes from a stream. If the kit is set to be of type <code>XMLEditorKit</code>, and the <code>desc</code> parameter is an
	 * <code>XMLDocument</code>, then it invokes the <code>XMLEditorKit</code> to initiate the read. Otherwise it calls the superclass method which loads the
	 * model as plain text.
	...
	/**
	 * This method invokes the <code>EditorKit</code> to initiate a read. In the case where a <code>ChangedCharSetException</code> is thrown this exception will
	 * contain the new CharSet. Therefore the <code>read</code> operation is then restarted after building a new Reader with the new charset.
	...
	*/
	...
	/**
	 * Fetches the editor kit to use for the given type of content. This version allows recognition of general XML (<code>*+xml</code>) content types that haven't
	 * been registered.
		...
		if(editorKit == null) { //if the editor kit is not for one of our recognized types
			...
			/*TODO integrate
						...
						if(editorKit==null) {	//if there is no registered editor kit
							...
							else if(isXML(ContentType.getInstance(type))) {	//if this type was not recognized, but it's an XML content type
	...
	*/
	...
	/**
	 * Updates the installed keymap based upon the current settings, such as whether the text pane is currently paged.
	 */
	...
	*/
	...
	/**
	 * Overrides the document being changed so that we can get fetch the new paged vew if necessary.
	...
	*/
	...
	/**
	 * Override of the <code>paint()</code> method to make sure a valid page is being shown.
	...
	public void paint(Graphics g) {
		...
		*/
		...
		if(mousePosition != null) { //if the mouse is over the component
			...
			final MouseEvent mouseEvent = new MouseEvent(this, MouseEvent.MOUSE_MOVED, System.currentTimeMillis(), 0, mousePosition.x, mousePosition.y, 0, false);
	...
	/**
	 * @return <code>true</code> if this text page is currently in the process of being paginated.
	 */
	...
	/**
	 * The variable used to remember the number of pages to display in case there is no paged view, yet.
	 */
	...
	/**
	 * Determines the absolute index from the beginning of allowable page display positions. This implementation compensates for the empty page slots on the first
	 * set of pages.
	...
	/**
	 * Determines the logical index of available pages. This implementation compensates for the empty page slots on the first set of pages.
	 * @param absolutePageIndex The zero-based index taking into account all page positions.
	 * @return The logical index of the position out of available pages, or <code>-1</code> if the given absolute page index does not correspond to an available
	 *         logical page index.
	 */
	...
	/**
	 * Navigates to the specified URI. If the URI is already loaded, it is displayed. If the URI is outside the publication, the location is loaded into the
	 * default browser.
	...
	public int search(final String searchText, int searchOffset) {
		if(searchOffset == NEXT_SEARCH_OFFSET) { //if they want to start searching after the results of the last search
			...
			if(searchOffset >= 0) //if we have a valid search offset (that is, the last search returned something valid
				...
				//if there was no search position last time, meaning we need to start from scratch
	...
	/**
	 * Invoked when a key has been typed. This event occurs when a key press is followed by a key release. Present to fulfill obligations of
	...
	/**
	 * Invoked when a key has been pressed. Creates necessary paging functionality in response to paging keys.
	 */
	/**
	 * TODO del when works public void keyPressed(KeyEvent e) { //TODO fix Debug.notify(KeyStroke.getKeyStrokeForEvent(e).toString()); //TODO testing }
	 */

	/**
	 * Invoked when a key has been released. Present to fulfill obligations of <code>KeyListener</code> interface.
	...
	/**
	 * Called when the displayed page has changed in the paged view, so that we can forward the event up the chain.
	...
	public AudioClip getAudioClip(final URL url) {
		...
		if(document instanceof XMLDocument) { //if this is an XML document
			...
			//if this is not an XML document, there's nothing we can do
	...
	/**
	 * Finds and returns the applet in the document represented by this applet context with the given name. The name can be set in the HTML tag by setting the
	 ...
	 * <tr>
	 * <td><code>"_top"</code>
	 * <td>Show in the top-level frame of the applet's window. If the applet's frame is the top-level frame, acts the same as "_self".
	 * </tr>
	 * <tr>
	 * <td><code>"_blank"</code>
	 ...
	 * <tr>
	 * <td><i>name</i>
	 * <td>Show in the frame or window named <i>name</i>. If a target named <i>name</i> does not already exist, a new top-level window with the specified name is
	...
	/**
	 * Requests that the argument string be displayed in the "status window". Many browsers and applet viewers provide such a window, where the application can
	...
	/**
	 * Associates the specified stream with the specified key in this applet context. If the applet context previously contained a mapping for this key, the old
	 * value is replaced.
	 * <p>
	...
	/**
	 * Returns the stream to which specified key is associated within this applet context. Returns <tt>null</tt> if the applet context contains no stream for this
	 * key.
	 * <p>
	 * For security reasons, mapping of streams and keys exists for each codebase. In other words, applet from one codebase cannot access the streams created by
	 * an applet from a different codebase
