...
/**
 * Abstract class that can decorate a user interface from component changes of a resource component manager.
...
public abstract class ResourceComponentDecorator<R extends Resource> {
	...
	public ResourceComponentManager<R> getResourceComponentManager() {
		return resourceComponentManager;
	}

	/**
	 * Constructs a decorator that uses the given resource component manager to manager resource components.
	...
	public ResourceComponentDecorator(final ResourceComponentManager<R> resourceComponentManager) {
		...
		resourceComponentManager.addResourceComponentListener(new ResourceComponentListener<R>() { //listen for resource component changes, and forward to our local methods
					...
					public void onResourceComponentAdded(final ResourceComponentManager<R>.ResourceComponentState resourceComponentState) {
						ResourceComponentDecorator.this.onResourceComponentAdded(resourceComponentState);
					}

					public void onResourceComponentRemoved(final ResourceComponentManager<R>.ResourceComponentState resourceComponentState) {
						ResourceComponentDecorator.this.onResourceComponentRemoved(resourceComponentState);
					}

					public void onResourceComponentSelected(final ResourceComponentManager<R>.ResourceComponentState oldResourceComponentState,
							final ResourceComponentManager<R>.ResourceComponentState newResourceComponentState) {
						ResourceComponentDecorator.this.onResourceComponentSelected(oldResourceComponentState, newResourceComponentState);
					}
				});
	}
	 ...
	 */
	protected abstract void onResourceComponentSelected(final ResourceComponentManager<R>.ResourceComponentState oldResourceComponentState,
			final ResourceComponentManager<R>.ResourceComponentState newResourceComponentState);
