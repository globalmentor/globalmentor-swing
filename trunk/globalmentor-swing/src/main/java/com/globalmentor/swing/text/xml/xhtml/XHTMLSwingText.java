...
public class XHTMLSwingText {
	...
	/**
	 * Determines if the specified attribute set represents an image. Specifically, this returns <code>true</code> if the XML element's name is "img"; or if the
	 * element's name is "object" and the type attribute is an image or the data attribute references an image file.
	 * @param attributeSet The attribute set which might represent an image.
	 * @return <code>true</code> if the specified attribute set represents an image, <code>false</code> if otherwise or the specified attribute set was
	 *         <code>null</code>.
	 */
	...
	/**
	 * Determines if the specified attribute set represents an HTML document. An element is considered to be a document element if one of the following are true:
	 * <ul>
	 * <li>The document has an HTML media type.</li>
	 * <li>The document element is an <code>&lt;html&gt;</code> element in an HTML namespace.</li>
	 * </ul>
	...
	/**
	 * Determines if the specified attribute set represents an HTML element. An element is considered to be an HTML element if one of the following are true:
	 * <ul>
	 * <li>The element is in an HTML namespace.</li>
	 * <li>The element is in no namespace but the document has an HTML media type.</li>
	 * <li>The element is in no namespace but the document element is an <code>&lt;html&gt;</code> element in an HTML namespace.</li>
	 * </ul>
	...
	/**
	 * Determines the reference to the image file represented by the attribute set, assuming the XML element (an "img" or "object" element) the attribute set
	 * represents does in fact represent an image. For "img", this return the "src" attribute. For objects, the value of the "data" attribute is returned.
	...
	/**
	 * Determines the media type of the given attribute set. It is assumed that the given attribute set is in fact for an object element.
	 * @param attributeSet The attribute set which represents an <code>&lt;object&gt;</code>.
	 * @return The media type of the data specified by the object attribute set, <code>null</code> if the media type could not be determined.
	 */
	public static ContentType getObjectMediaType(final AttributeSet attributeSet) {
		if(attributeSet != null) { //if there are attributes
			...
			if(codeType != null) //if the object has a code type
				return ContentType.create(codeType); //create a media type from the given type
			...
			if(type != null) //if the object has a code type
				return ContentType.create(type); //create a media type from the given type and return it
