...
public class XEBEditorKit extends XHTMLEditorKit //TODO eventually go back to deriving from XMLEditorKit and use an appropriate editor kit for each document in the spine
{
	 ...
	 */
	public Object clone() {
		return new XEBEditorKit(getURIInputStreamable());
	}
	...
	public String getContentType() {
		return ContentType.toString(ContentType.APPLICATION_PRIMARY_TYPE, ContentTypeConstants.X_XEBOOK_RDF_XML_SUBTYPE);
	}
	...
	public void read(InputStream inputStream, Document document, int pos) throws IOException, BadLocationException {
		if(document instanceof XMLDocument) { //if this is a Swing XML document
			...
			final org.w3c.dom.Document xmlDocument = XML.parse(inputStream, swingXMLDocument.getBaseURI(), true, new URIInputStreamableXMLEntityResolver(
					swingXMLDocument.getURIInputStreamable())); //parse the document using the input stream locator of the document for loading other needed documents
	...
	protected void read(final RDF rdf, final XMLDocument swingXMLDocument, int pos) throws IOException, BadLocationException {
		...
		if(publication != null) { //if there is a book
			...
			*/
			final DocumentBuilder documentBuilder = XML
					.createDocumentBuilder(true, new URIInputStreamableXMLEntityResolver(swingXMLDocument.getURIInputStreamable())); //create a document builder using the input stream locator of the document for loading other needed documents
